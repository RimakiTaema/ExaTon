...
class ServersApi {
  ...
  ///   Unique server ID
  Future<Response> getServerWithHttpInfo(String serverId,) async {
    // ignore: prefer_const_declarations
    final path = r'/servers/{serverId}/'
  ...
  Future<GetServer200Response?> getServer(String serverId,) async {
    final response = await getServerWithHttpInfo(serverId,);
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'GetServer200Response',) as GetServer200Response;
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getServersWithHttpInfo() async {
    // ignore: prefer_const_declarations
  ...
  Future<GetServers200Response?> getServers() async {
    final response = await getServersWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
