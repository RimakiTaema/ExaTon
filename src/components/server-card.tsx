...
import { CubeIcon, ShareNetworkIcon, UsersIcon } from "@phosphor-icons/react";
...
export function ServerCard({ server }: { server: ServerData }) {
  ...
  return (
    <Link href={`/home/servers?id=${server.id}`}>
      <Card
        className="cursor-pointer transition-shadow hover:shadow-md hover:ring-emerald-300/50"
        size="sm"
      >
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              <span
                className={`size-2 rounded-full ${statusInfo.dot} ${isOnline ? "animate-pulse" : ""}`}
              />
